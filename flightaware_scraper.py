...
out = bpath+'/'+AD+'/csv/2019/ref/'+bname +'.csv'
...
for row in table:
        ...
        if len(cells) ==10 and cells[7].text.encode('utf-8') is not '':
            ...
            #print 6,cells[6].text.encode('utf-8')
            #print 6,cells[6].text.encode('utf-8').replace(",", " ").split()[-1:][-4:]
            ...
            #j = '%i,%s,%f,%f,%s,%s,%s,%s,%s' % (count, cells[0].text.encode('utf-8'), float(cells[1].text.encode('utf-8')[:7]),   float(cells[2].text.encode('utf-8')[:8]), replaceMultiple(cells[3].text.encode('utf-8'),['\xc2', '\xe2', '\xC3', '\xC2', '&searr;','&rarr;', '&nearr;', '&swarr', '&nwarr','&deg;'], ""), cells[4].text.encode('utf-8'),(cells[5].text.encode('utf-8')), cells[6].text.encode('utf-8').replace(",", "") * 1000, replaceMultiple(cells[7].text.encode('utf-8'), [',', '\xc2', '\xe2', '\xC3', '\xC2',';'], ""))
            j = '%i,%s,%f,%f,%s,%s,%s,%s,%s' % (count, cells[0].text.encode('utf-8')[:-7], float(cells[1].text.encode('utf-8')[:7]),float(cells[2].text.encode('utf-8')[:8]), replaceMultiple(cells[3].text.encode('ascii',errors='ignore'),[';','\xef','\xc2', '\xe2', '\xC3', '\xC2','&searr', '&searr;','&rarr;', '&nearr;', '&swarr', '&nwarr','&deg;'], ""), cells[4].text.encode('utf-8'),(cells[5].text.encode('utf-8')), str(cells[6].text.encode('ascii',errors='ignore')).replace(',','')[:int(len((str(cells[6].text.encode('ascii',errors='ignore')).replace(',', '')))*.5)], replaceMultiple(cells[7].text.encode('ascii',errors='ignore'), ['\xef',',', '\xc2', '\xe2', '\xC3', '\xC2',';'], ""))
            print j
